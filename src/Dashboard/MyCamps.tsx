...
// import { MapPin, Users, Eye, Trash2, ChevronLeft, ChevronRight, Image as ImageIcon, Star, CheckCircle2 } from "lucide-react";
// import { api } from "../lib/api";
// import { toast } from "sonner";
// import AddCampRequestWizard from "./AddCampRequestWizard";

// // ثوابت وتصنيفات لقسم الإضافة
// const propertyTypes = ["خيمة","نُزل","عريش","كرافان","بود","غلمبينغ"];
// const ARAB_COUNTRIES = [
//   { code: "DZ", label: "الجزائر (Algeria)" }, { code: "BH", label: "البحرين (Bahrain)" },
//   { code: "KM", label: "جزر القمر (Comoros)" }, { code: "DJ", label: "جيبوتي (Djibouti)" },
//   { code: "EG", label: "مصر (Egypt)" }, { code: "IQ", label: "العراق (Iraq)" },
//   { code: "JO", label: "الأردن (Jordan)" }, { code: "KW", label: "الكويت (Kuwait)" },
//   { code: "LB", label: "لبنان (Lebanon)" }, { code: "LY", label: "ليبيا (Libya)" },
//   { code: "MR", label: "موريتانيا (Mauritania)" }, { code: "MA", label: "المغرب (Morocco)" },
//   { code: "OM", label: "عُمان (Oman)" }, { code: "PS", label: "فلسطين (Palestine)" },
//   { code: "QA", label: "قطر (Qatar)" }, { code: "SA", label: "السعودية (KSA)" },
//   { code: "SO", label: "الصومال (Somalia)" }, { code: "SD", label: "السودان (Sudan)" },
//   { code: "SY", label: "سوريا (Syria)" }, { code: "TN", label: "تونس (Tunisia)" },
//   { code: "AE", label: "الإمارات (UAE)" }, { code: "YE", label: "اليمن (Yemen)" },
// ];
// const documentTypes = [
...
//           {facilities.map((item) => (
//             <div key={item} className="flex items-center space-x-2 space-x-reverse">
//               <Checkbox id={`facility-${item}`} checked={(data.features.facilities || []).includes(item)} onCheckedChange={() => toggleFacilities(item)} />
...
//   const BED_LABELS: Record<string, string> = { king: "سرير كينغ", queen: "سرير كوين", double: "سرير مزدوج", twin: "سرير مفرد", bunk: "سرير بطابقين", sofa: "سرير أريكة", crib: "سرير أطفال", air: "سرير هوائي" };
...
//       const facilities = toArray(j.facilities);
...
//       const bedsArray = Object.keys(bedsObj).map((k) => ({ key: k, label: BED_LABELS[k] || k, count: Number(bedsObj[k] || 0) })).filter((b) => b.count > 0);
...
//           zip: location.zip || "", street: location.street || "",
//         },
//         capacity: { maxGuests: Number(capacity.maxGuests ?? 0), bedrooms: Number(capacity.bedrooms ?? 0), beds: bedsArray },
//         facilities,
//         amenities: {
//           basic: toArray(amenities.basic), bath: toArray(amenities.bath),
//           kitchen: toArray(amenities.kitchen), outdoor: toArray(amenities.outdoor),
//         },
//         sharedSpaces,
//         seclusion,
//         activities: { options: toArray(activities.options) },
//         terrain: terrainArr,
//         rules: {
//           checkInFrom: rules.checkInFrom || "", checkInTo: rules.checkInTo || "",
//           checkOut: rules.checkOut || "", minAge: Number(rules.minAge ?? 0),
...
//   // قراءة status من data.request.status
...
//       {/* Details Dialog — يعرض كل الحقول من merged.rawPayloadJson */}
